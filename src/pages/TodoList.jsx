...
function TodoList() {
    ...
    useEffect(() => {
        fetch("https://jsonplaceholder.typicode.com/posts?_limit=10") // берём только 10, чтобы не перегружать
            .then((response) => {
                if (!response.ok) throw new Error("Ошибка загрузки данных");
                return response.json();
            })
            .then((posts) => {
                // Преобразуем посты в формат наших задач
                const loadedTasks = posts.map((post) => ({
                    id: post.id,
                    title: post.title,
                    completed: false, // или Math.random() > 0.7 для разнообразия
                }));
            ...
            >
                ...
                <div
                    className={`scroll ${darkTheme ? "bg-gray-800" : "bg-white"
                        } w-full h-[400px] md:h-[500px] px-2 overflow-y-scroll flex items-center justify-center rounded-md shadow-lg relative transition-all duration-500`}
                >
                    <div
